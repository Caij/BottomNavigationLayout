...
public class NavigationItemView extends FrameLayout {
    ...
    public void initialise(NavigationItem tabItem) {
        ivIcon.setSelected(false);
        activeColor = tabItem.getInactiveColor() > 0 ? tabItem.getInactiveColor() : fetchContextColor(getContext(), R.attr.colorAccent);
        inActiveColor = tabItem.getActiveColor() > 0 ? tabItem.getInactiveColor() : Color.LTGRAY;
